...
var positiondetermined = 0;

world.beforeEvents.playerBreakBlock.subscribe((data) => {
    var _a, _e;
    data.cancel = protect(data.player, data.block.location);
    if (!protect(data.player, data.block.location) && data.itemStack && ((_a = data.itemStack.nameTag) === null || _a === void 0 ? void 0 : _a.toLowerCase()) === "claim wand" && positiondetermined === 0) {
        PlayerLandPos.setPos2(data.player, data.block.location, data.block.dimension.id);
        data.player.sendMessage(`[Land] §aSet 1`);
        data.cancel = true;
    }
    else if (!protect(data.player, data.block.location) && data.itemStack && ((_e = data.itemStack.nameTag) === null || _e === void 0 ? void 0 : _e.toLowerCase()) === "claim wand" && positiondetermined === 1) {
        PlayerLandPos.setPos1(data.player, data.block.location, data.block.dimension.id);
        data.player.sendMessage(`[Land] §aSet 2`);
        data.cancel = true
        positiondetermined = 2;
    }
    if (positiondetermined === 0) {
        positiondetermined = 1;
    }
    else if (positiondetermined === 2) {
        positiondetermined = 0;
    }
});
